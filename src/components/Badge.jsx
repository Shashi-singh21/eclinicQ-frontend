...
const Badge = ({
  ...
  // Style by type
  const base = 'inline-flex items-center gap-1 border font-medium select-none transition-colors';
  const ghost = cn(t.text, t.border, t.bg, hover ? t.hoverBg : '');
  const solid = cn('text-white',
    resolvedColor === 'blue' && 'bg-blue-600 border-blue-600 hover:bg-blue-700',
    resolvedColor === 'green' && 'bg-green-600 border-green-600 hover:bg-green-700',
    resolvedColor === 'orange' && 'bg-orange-600 border-orange-600 hover:bg-orange-700',
    resolvedColor === 'yellow' && 'bg-amber-500 border-amber-500 hover:bg-amber-600',
  ...
  return (
    <Component
      className={cn(base, sizeCls, appearance, disabled ? 'opacity-60 pointer-events-none cursor-not-allowed' : '', className)}
