...
export default function SignIn({ variant = 'neutral' }){
  ...
  const requestOtp = async () => {
    ...
    try {
      ...
      const cid = resp?.data?.data?.challengeId || resp?.data?.challengeId || resp?.challengeId
      setChallengeId(cid || null)
      setOtpSent(true)
      resetOtp()
    } catch (e) {
      const apiMsg = e?.response?.data?.message || e?.response?.data?.error || e?.message || 'Failed to send OTP'
  ...
  const verifyOtp = async () => {
    ...
    setErrorMsg('')
    try {
      const data = await loginOtpVerify({ challengeId, userName: identifier.trim(), otp: otp.join('') })
  ...
  const canPasswordLogin = identifier.trim().length > 0 && password.trim().length > 0
  const canMPinLogin = identifier.trim().length > 0 && mpin.trim().length === 4
  const canLogin = mode === 'password' ? canPasswordLogin : mode === 'mpin' ? canMPinLogin : false
  ...
  const handlePasswordLogin = async () => {
    ...
    setErrorMsg('')
    try {
      const data = await loginPassword({ userName: identifier.trim(), password: password })
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="relative flex-1 flex items-center justify-center px-6 pb-10 md:px-10 md:pb-10">
        <div className="absolute left-8 md:left-12 top-8 md:top-8">
          ...
          {copy.badge && (
            <div className="text-[11px] text-[#F59E0B] mt-0.5">{copy.badge}</div>
          )}
        </div>
        <div className="w-full max-w-[520px] relative -top-2 md:-top-4">

          <h1 className="text-2xl font-semibold text-gray-800">{copy.heading}</h1>
          ...
          <div className="space-y-3 mt-4">
            {errorMsg && (
              <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded px-3 py-2">{errorMsg}</div>
            )}
            ...
            {mode==='password' && (
              <div>
                ...
                <div className="mt-1 relative">
                  <input className="h-9 w-full border border-gray-300 rounded px-2 text-sm pr-8" placeholder="Enter Password" type="password" value={password} onChange={(e)=>setPassword(e.target.value)} onKeyDown={(e)=>{ if(e.key==='Enter' && canPasswordLogin) handlePasswordLogin() }} />
            ...
            {mode==='mpin' && (
              <div>
                ...
                <input className="mt-1 h-9 w-full border border-gray-300 rounded px-2 text-sm" placeholder="••••" inputMode="numeric" maxLength={4} value={mpin} onChange={(e)=>setMpin(e.target.value.replace(/\D/g,'').slice(0,4))} />
              </div>
            )}
              ...
              <div>
                {!otpSent ? (
                  <div className="space-y-2">
                    ...
                    <button disabled={!identifier || otpRequesting} onClick={requestOtp} className={`w-full h-9 rounded text-sm flex items-center justify-center border ${(identifier && !otpRequesting) ? 'bg-[#2F66F6] text-white border-[#2F66F6]' : 'bg-[#F3F4F6] text-[#9AA1A9] border-[#E5E7EB] cursor-not-allowed'}`}>
                      {otpRequesting ? 'Sending...' : (<><span className="mr-1">💬</span> Send OTP</>)}
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    <button disabled={!allOtpFilled || submitting} onClick={verifyOtp} className={`w-full h-9 rounded text-sm flex items-center justify-center ${(allOtpFilled && !submitting) ? 'bg-[#2F66F6] text-white' : 'bg-[#F3F4F6] text-[#9AA1A9]'}`}>
            ...
            {mode!=='otp' && (
              <Button className="w-full" disabled={!canLogin || submitting} onClick={()=>{
